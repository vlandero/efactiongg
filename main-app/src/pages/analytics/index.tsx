...
function ChampionStats({
  ...
  calculateWinrate: (champ: string) => string;
}) {
    ...
    <div className="space-y-4">
      ...
      {selectedChampion && (
        <div className="text-lg">
          Winrate for <strong>{selectedChampion}</strong>:{" "}
          {calculateWinrate(selectedChampion)}
        </div>
      )}
    </div>
  );
...
export default function AnalyticsPage({
  ...
  const [games, setGames] = useState<Game[]>(
    Object.entries(categorizedGames).flatMap(([category, games]) =>
      categories[category] ? games : []
    )
  );
  const [selectedTab, setSelectedTab] = useState<"matches" | "statistics">(
    "matches"
  );
  ...
  const refreshGames = () => {
    const filteredGames = Object.entries(categorizedGames).flatMap(
      ([category, games]) => (categories[category] ? games : [])
    );
  ...
  const removeGame = (id: string) => {
    for (const category of Object.keys(categorizedGames)) {
      categorizedGames[category] = categorizedGames[category].filter(
        (game) => game.id !== id
      );
    }
    refreshGames();
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex gap-4 flex-wrap">
          {Object.keys(categories).map((cat) => (
            <div
              key={cat}
              className="flex items-center gap-2 bg-neutral-800 px-3 py-1 rounded"
            >
              <input
                type="checkbox"
